...
export function MentorDisciplinasPage() {
  ...
  const [disciplinaParaRemover, setDisciplinaParaRemover] = useState<Disciplina | null>(
    null,
  )
  ...
  const cadastrarEAdicionar = async () => {
    ...
    try {
      ...
      await adicionarDisciplinaAoUsuario(mentorId, nova.id)
  ...
  const remover = async () => {
    if (mentorId === undefined || !disciplinaParaRemover) return
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-foreground">Minhas Disciplinas</h1>
          <p className="text-muted-foreground">
            Gerencie as disciplinas que você domina
          </p>
        </div>
          ...
          {carregando ? (
            ...
            <div className="text-center py-8">
              ...
              <p className="text-sm text-muted-foreground mt-1">
                Adicione disciplinas para que mentorados possam te encontrar.
              </p>
            </div>
          ) : (
